...
// Автоматический расчет текстов и дедлайна на месяц раньше (Не менять)
const _monthsRU = ["января", "февраля", "марта", "апреля", "мая", "июня", "июля", "августа", "сентября", "октября", "ноября", "декабря"];
const _monthsData = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
...
CONFIG.getRSVPTextDate = function() {
    ...
    rsvpDate.setMonth(rsvpDate.getMonth() - 1); // Строго на месяц раньше даты свадьбы
    return `${rsvpDate.getDate()} ${_monthsRU[rsvpDate.getMonth()]} ${rsvpDate.getFullYear()} года`;
};

CONFIG.getTimerTargetString = function() {
    return `${_monthsData[this.WEDDING_DATE.getMonth()]} ${this.WEDDING_DATE.getDate()}, ${this.WEDDING_DATE.getFullYear()} ${this.WEDDING_DATE.getHours()}:${this.WEDDING_DATE.getMinutes()}:${this.WEDDING_DATE.getSeconds()}`;
};
